...
int main(int argc, char *argv[])
{
    ...
    Coordinates CoordinatesCluster_(Parameters_.lx_cluster,
                                    Parameters_.ly_cluster, Parameters_.n_orbs);
    ...
    if (ex_string == "NI")
    {
        ...
        cout << "mu for n=4 = "
             << 0.5 * (Hamiltonian_.eigs_[temp - 1] + Hamiltonian_.eigs_[temp])
             << "   " << Hamiltonian_.eigs_[temp - 1] << "   "
             << Hamiltonian_.eigs_[temp] << endl;
        Parameters_.mus =
            0.5 * (Hamiltonian_.eigs_[temp - 1] + Hamiltonian_.eigs_[temp]);
    ...
    else if (ex_string == "sf")
    {
        ...
        cout << setprecision(9);
        MCEngine MCEngine_(Parameters_, Coordinates_,
                           MFParams_, Hamiltonian_, Observables_);
    ...
    { //observe
        ...
        n_states_occupied_zeroT =
            Coordinates_.nbasis_ *
            2.0 * (Parameters_.Fill / (Parameters_.n_orbs * 2.0));

        if (!Parameters_.fixed_mu_value)
        {
            initial_mu_guess =
                0.5 * (Hamiltonian_.eigs_[n_states_occupied_zeroT - 1] +
                       Hamiltonian_.eigs_[n_states_occupied_zeroT]);
        }
        else
        {
        ...
        Parameters_.mus =
            Hamiltonian_.chemicalpotential(initial_mu_guess, Parameters_.Fill);
